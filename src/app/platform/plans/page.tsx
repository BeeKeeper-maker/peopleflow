...
export default function PlansPage() {
    ...
    if (loading) {
        return <div className="h-72 rounded-xl bg-hover animate-pulse" />;
    }
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
                {plans.map((plan, i) => {
                    ...
                    return (
                        <div key={plan.id} className="rounded-xl border border-border bg-hover/50 overflow-hidden hover:border-border transition-all group">
...
function Metric({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
    return <div className="rounded-xl border border-border bg-hover/50 p-4"><Icon className="mb-3 h-5 w-5 text-indigo-400" /><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 text-xl font-bold text-foreground">{value}</p></div>;
}

function Input({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
    return <label className="block text-xs font-medium text-foreground">{label}<input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="mt-1 w-full rounded-lg border border-border bg-hover px-3 py-2 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:border-indigo-400/50" /></label>;
}

function MoneyInput({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
    return <label className="text-xs text-foreground/60">{label}<input type="number" value={Math.round(value / 100)} onChange={(e) => onChange(Number(e.target.value || 0) * 100)} className="mt-1 w-full rounded-lg border border-border bg-black/10 px-2 py-1.5 text-sm text-foreground outline-none" /><span className="text-[10px] text-foreground/40">{taka(value)}</span></label>;
}
...
function NumberInput({ icon: Icon, label, value, onChange }: { icon: React.ElementType; label: string; value: number; onChange: (v: number) => void }) {
    return <label className="rounded-lg border border-border bg-hover p-2 text-xs text-foreground"><span className="flex items-center gap-1.5"><Icon className="h-3.5 w-3.5" />{label}</span><input type="number" value={value} onChange={(e) => onChange(Number(e.target.value || 0))} className="mt-1 w-full bg-transparent text-sm font-medium text-foreground outline-none" /></label>;
}
