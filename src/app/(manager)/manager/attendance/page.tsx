...
import { useManagerTeam, useManagerAttendance, type ManagerAttendanceRecord, type ManagerTeamMember } from "@/hooks/use-data";
...
export default function ManagerAttendancePage() {
    ...
    // ── TanStack Query: team + attendance for the selected month ──
    const year = currentMonth.getFullYear();
    const month = currentMonth.getMonth() + 1;
    const { data: employees = [], isLoading: teamLoading } = useManagerTeam();
    const { data: attendanceRecords = [], isLoading: attendanceLoading, isFetching, refetch } = useManagerAttendance({ year, month, limit: 1000 });

    const isLoading = teamLoading || attendanceLoading;

    const teamAttendance: TeamMemberAttendance[] = useMemo(() => {
        const today = businessDateKey(new Date());
        const workingDaysInMonth = getWorkingDaysInMonth(year, month);

        return (employees as ManagerTeamMember[]).map((emp) => {
            const empAttendance = (attendanceRecords as ManagerAttendanceRecord[]).filter((record) => record.employeeId === emp.id);
    ...
    }, [employees, attendanceRecords, year, month, getWorkingDaysInMonth, t]);
