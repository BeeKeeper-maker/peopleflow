 ...
 *
 * ✅ Performance optimized + RLS-scoped via withDB:
 *  - All queries run inside withTenant() transaction
...
export async function GET(req: Request) {
    try {
        ...
        const data = await ctx.withDB(async (db) => {
            // ============================================
            ...
            const allPayrolls = await db.salarySlip.findMany({
                where: {
                    employee: { organizationId: orgId },
