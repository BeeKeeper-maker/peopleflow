...
export async function POST(request: Request) {
  try {
    ...
    // Find valid token
    const resetToken = await withPlatform((db) =>
    ...
    const passwordHistory = await withPlatform((db) =>
      db.passwordHistory.findMany({
    ...
    const allHistory = await withPlatform((db) =>
      db.passwordHistory.findMany({
        where: { userId: user.id },
        orderBy: { createdAt: "desc" },
      }),
    ...
    if (user.email) {
      sendEmail({
        ...
        authLogger.error({ err: err }, "Failed to send password change email:"),
      );
    }

    const responseMessage =
      resetToken.purpose === "employee_invitation"
