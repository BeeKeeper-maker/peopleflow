...
export async function POST(request: Request) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Scope the rest of onboarding to the new tenant. Production RLS uses
      // forced policies, so all tenant-owned bootstrap rows must be created
      // after setting app.current_tenant_id inside this transaction.
      ...
      const trialEnd = new Date(
        trialStart.getTime() + 1000 * 60 * 60 * 24 * 14,
      );
