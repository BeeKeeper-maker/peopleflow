...
export async function requireAuth(): Promise<AuthContext | NextResponse> {
  try {
    ...
    // Session validation starts with only an email/id, before tenant context
    // exists. Use a controlled RLS bypass, then enforce tenant/org rules below.
...
export async function requireEmployee(): Promise<
  (AuthContext & { employeeId: string }) | NextResponse
> {
  const authResult = await requireAuth();
  if (authResult instanceof NextResponse) return authResult;
  if (!authResult.employeeId) return AuthErrors.NO_EMPLOYEE();

  const employee = await authResult.withDB((db) =>
...
export async function requireManagerOrAbove(): Promise<
