...
export function DesignationActions({ id }: DesignationActionsProps) {
    const router = useRouter()
    const { addToast } = useToast()
    const [showDeleteAlert, setShowDeleteAlert] = useState(false)
    const [isDeleting, setIsDeleting] = useState(false)
    const t = useTranslations("SharedComponents.designationActions")
        ...
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="bg-background border-card-border">
                    <DropdownMenuItem onClick={() => router.push(`/designations/${id}/edit`)}>
